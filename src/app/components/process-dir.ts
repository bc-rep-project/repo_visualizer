...
export function processDirectoryData(data: any[]): { root: DirectoryNode, importLinks: { source: FileNode, target: FileNode }[], dependencies: Dependencies } {
  ...
  data.forEach((file) => {
    ...
    if (file.content) {
      ...
      console.log("Extracted imports from", fileName, ":", imports); // Debug extracted imports
      const fileNode: FileNode = {
        name: fileName,
        imports: imports.map(importName => ({ moduleName: importName.moduleName, sourceFile: file.path })),
        type: fileExt,
        x: 0,
        y: 0,
      };
      ...
      imports.forEach((importName) => {
        ...
        if (importNode) {
          console.log("Found matching node:", importNode.name); // Debug matching node
          ...
          console.log("No matching node found for import:", importName.moduleName); // Debug missing node
        }
      });
...
function findImportNode(importName: string, node: DirectoryNode): FileNode | undefined {
  console.log("Searching for:", importName); 
  ...
  if ('children' in node) {
    for (const child of node.children) {
      ...
      } else if ((child as FileNode).name.split('.')[0] === importName) { // Compare without extension
