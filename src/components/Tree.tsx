...
export const Tree = (
  ...
  const { colorScale, colorExtent } = useMemo(() => {
    ...
    const flattenTree = (d) => d.children ? d.children.flatMap(flattenTree) : d;
    ...
    if (Array.isArray(colorExtent) && colorExtent.length === 2 && !isNaN(colorExtent[0]) && !isNaN(colorExtent[1])) {
      const colorScale2 = scaleLinear()
        ...
        const getColor = (value) => colors[Math.round(colorScale2(value))];
  ...
  const selectedNode = selectedNodeId && packedData.find((d) => d && d.data.path === selectedNodeId);
...
//   sortOrder?: number;
//   fileColors?: { [key: string]: string };
//   nodeId: string; // Added unique node ID
// } & FileType;
...
// const height = 1000;
// const maxChildren = 9000;
// const lastCommitAccessor = (d: ExtendedFileType) =>
//   new Date(d.commits?.[0]?.date + "0");
// const numberOfCommitsAccessor = (d: ExtendedFileType) =>
//   d?.commits?.length || 0;
...
//   const { colorScale, colorExtent } = useMemo(() => {
//     if (!data) return { colorScale: () => {}, colorExtent: [0, 0] };
//     const flattenTree = (d: ExtendedFileType) => {
...
//   const getColor = (d: ExtendedFileType) => {
//     if (colorEncoding === "type") {
//       const isParent = d.children;
//       if (isParent) {
//         const extensions = countBy(d.children, (c) => c.extension);
//         const mainExtension = maxBy(entries(extensions), ([k, v]) => v)?.[0];
//         return mainExtension && fileColors[mainExtension]
//           ? fileColors[mainExtension]
//           : "#CED6E0";
//       }
//       return fileColors[d.data.extension] || "#CED6E0";
//     } else if (colorEncoding === "number-of-changes") {
//       return colorScale(numberOfCommitsAccessor(d)) || "#f4f4f4";
//     } else if (colorEncoding === "last-change") {
//       return colorScale(lastCommitAccessor(d)) || "#f4f4f4";
...
//     const hierarchicalData = hierarchy(
//       processChild(data, getColor, cachedOrders.current, 0, fileColors),
//       (d) => d.children, // Access children directly
//     )
//       .sum((d) => d.value ?? 0)
//       .sort((a, b) => {
...
//           return -1;
//         }
//         if (a.data.path.startsWith("src/fonts")) {
//           return 1;
//         }
...
//     const saveCachedPositionForItem = (item: ProcessedDataItem) => {
//       cachedOrders.current[item.data.path] = item.data.sortOrder ?? 0;
//       if (item.children) {
//         item.children.forEach(saveCachedPositionForItem);
//       }
//     };
...
//   const hoveredFileNode =
//     hoveredFileId &&
//     packedData.find((d) => d.data.path === hoveredFileId);

//   const fileTypes = uniqBy(
//     packedData.map(
//       (d) => fileColors[d.data.extension] && d.data.extension,
//     ),
//   )
//     .sort()
//     .filter(Boolean) as string[];

//   const showConnectionsForFile = (fileId: string) => {
...
//         </filter>
//       </defs>
//       {hoveredFileNode?.data?.imports?.map((importItem, i) => {
//         // find the imported file
//         const importedFileNode = packedData.find(
//           (d) =>
//             d.data.pathWithoutExtension === importItem.moduleName,
//         );
//         if (!importedFileNode) return null;

//         return (
//           <line
//             key={i}
//             x1={hoveredFileNode.x}
//             y1={hoveredFileNode.y}
//             x2={importedFileNode.x}
//             y2={importedFileNode.y}
//             style={{
//               stroke: "#29081959",
//               strokeWidth: "2",
//               pointerEvents: "none",
...
//         return (
//           <g
//             key={data.nodeId} // Use unique ID as key
//             style={{
//               fill: doHighlight
//                 ? isHighlighted
//                   ? "#FCE68A"
...
//             }}
//             transform={`translate(${x}, ${y})`}
//             onMouseEnter={() => showConnectionsForFile(data.path)}
