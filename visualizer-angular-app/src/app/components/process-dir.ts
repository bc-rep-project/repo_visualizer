...
interface D3Node extends d3.HierarchyNode<DirectoryNode | FileNode> {
  x: number;
  y: number;
}
...
export function processDirectoryData(data: any[]): { root: D3Node, importLinks: { source: D3Node, target: D3Node }[] } {
  ...
  const nodesMap: { [key: string]: D3Node } = {}; // Change to D3Node
  ...
  const createOrGetNode = (name: string, parent?: D3Node, type?: string, imports?: ImportObject[]): D3Node => {
    ...
    const newNode: D3Node = {
      data: { 
        name, 
        children: type === 'dir' ? [] : undefined, // Only directories have children
        imports: imports || [], 
        type: type || 'file',
        x: 0, 
        y: 0 
      },
      // ... other d3.HierarchyNode properties
      children: [],
